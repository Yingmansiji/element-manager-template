...
axios.interceptors.response.use(
  ...
  error => {
    ...
    if (error && error.response) {  // 根据响应码处理
      switch (error.response.status) {  // 注意：error.response.status的类型是number
        case 400:
          Message.error('这是个错误请求')
          break;
        case 401:
          // 401 unauthorized，表示发送的请求需要有通过 HTTP 认证的认证信息
          // 401通常可以用于捕获302重定向操作，因为3xx的状态码前端捕获不到，但401可以被捕获
          // 注意这里需要后端配合，将响应的重定向状态码302重置为401，
          Message.error('需要用户进行身份认证')
      ...
      if (error.message.indexOf('timeout') !== -1) {  // 超时处理
...
function post(url, params,header) {
  return new Promise((resolve, reject) => {
    axios.post(url, Qs.stringify(params),{headers:header})
      .then(res => {
        resolve(res.data);
      })
      .catch(err => {
